...
> **Status:** Tranche 0 plan. No config files yet. They will land here as the spine and tooling boot up.
...
| `toolbox_manifest.json` | High-level descriptor of the entire `.scaffold/` toolbox for **zero-context agent entry**. First file an agent reads after MCP handshake. See [`toolbox_manifest.json.PLAN.md`](toolbox_manifest.json.PLAN.md). | `install_orchestrator` on first run; refreshed on tool register/unregister and contract revision. |
