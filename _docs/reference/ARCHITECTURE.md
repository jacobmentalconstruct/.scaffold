...
> **Status:** Revised through T9. The substrate baseline is now proven in both development scope and fresh installed-project scope. Folds in the Memory Model + Operational Rituals + Cross-cutting Principles + Constraint Registry from precursor doctrine review (see [`_docs/migration/INCORPORATION_INVENTORY.md`](../migration/INCORPORATION_INVENTORY.md)). The original Tranche 0 design (sections 1–11 below, renumbered) is preserved where it still matches shipped behavior; later tranches update the document as the substrate becomes real.
...
- **Future — awareness of the plan.** The agent also carries forward intent: the current tranche scope, the next step in [`_docs/planning/IMPLEMENTATION_ROADMAP.md`](../planning/IMPLEMENTATION_ROADMAP.md), the goals not yet met, what comes next.
...
The "plan" is itself a piece of LTM, but it has a special role: it is the only LTM content that is *forward-looking by nature*. The journal and events record the past; the projections describe the present; the plan describes the intended future. Keeping the agent's perception of the plan consistent with what the journal and event log show is the job of the bootstrap projection's forward fields — they're regenerated every time `_docs/planning/IMPLEMENTATION_ROADMAP.md` revises or a tranche closes.
...
| **`close_tranche`** envelope | `closeout_orchestrator.py` | Reads the ledger, compiles park notes, writes `_docs/history/tranches/Tn_PARK_NOTES.md`, creates + closes tranche journal entry, seals ledger |
...
- The compiled park notes (`_docs/history/tranches/Tn_PARK_NOTES.md`)
...
Nothing mutates the host project at this stage. See [`_docs/planning/IMPLEMENTATION_ROADMAP.md`](../planning/IMPLEMENTATION_ROADMAP.md) for the tranche order to reach this loop end-to-end.
...
| Design | This document | `/_docs/reference/ARCHITECTURE.md` |
...
5. **Capture.** Write `_docs/history/tranches/T_n_PARK_NOTES.md` (code-time mirror) capturing the inspect output, decisions made at code time, files touched, proving-loop status. Put the file's bytes in `blob_store` via `state.blob_store.put`; record the SHA-256 hash and the post-capture Merkle root.

6. **Journal.** Dispatch a `create_journal_entry` envelope with:
    - `kind='tranche'`
    - title `"T_n <subject> — COMPLETE"`
    - body from `_docs/history/tranches/T_n_PARK_NOTES.md`
...
7. **Update continuity docs (explicit checklist).** Every item below must be touched, otherwise smoke test drift-detection (step 8) will fail:
    - **`_docs/continuity/ONBOARDING.md`** — reading order and verification commands reflect current surfaces.
    - **`_docs/continuity/WE_ARE_HERE_NOW.md`** — fast pickup note reflects the latest parked tranche, any current tranche, and the next horizon with unambiguous wording.
    - **`_docs/planning/TARGET_STATE.md`** — update whenever the target state, routing doctrine, chat/Tk role split, or tranche sequence changes.
    - **`_docs/planning/NORTHSTARS.md`** — satisfied substrate capabilities and next horizons updated.
    - **`_docs/history/DEV_LOG.md`** — append-only milestone narrative extended for the tranche.
    - **`_docs/planning/IMPLEMENTATION_ROADMAP.md`** — mark the tranche `✓ COMPLETE` with metrics + evidence hash + the tranche journal entry's uid + the `task_completed` event id.
    - **`_docs/reference/SOURCE_PROVENANCE.md`** — append a dated entry distinguishing original code from structurally-borrowed patterns; cite evidence hashes; record the new journal entry uid.
    - **`_docs/reference/TOOLS.md`** — regenerate/update so the registered-tools table row count equals `tool_registry` table row count. Any tools added in the tranche get a row here.
    - **Generated closeout metadata** — `close_tranche` writes `_docs/continuity/LATEST_PARKED_TRANCHE.json/.md` plus a tranche-specific `_docs/history/tranches/Tn_CLOSEOUT_METADATA.json/.md` file so exact closeout ids and CAS refs are derived mechanically instead of hand-copied into mirror docs.
    - **`_docs/reference/ARCHITECTURE.md §15`** — add a `Resolved at T_n` subsection listing the open questions this tranche resolved. Carry the still-open list forward.
...
8. **Re-verify.** Run `python smoke_test.py` again. The drift-detection sections verify mechanically that step 7 was performed:
    - `_docs/continuity/WE_ARE_HERE_NOW.md` exists and names the latest parked tranche plus either the current tranche or the next horizon, depending on lifecycle state.
    - `_docs/history/DEV_LOG.md` exists and contains an entry for the latest closed tranche.
    - `_docs/planning/NORTHSTARS.md` exists and reflects the current next horizon.
...
Without mechanical enforcement, Park Phase discipline drifts back into conversational ritual within a few sessions. The active contract clause (`BCC.md §10.8 Park Phase closure rule`, historically `§D Park Phase Discipline`) makes it a HARD_BLOCK gate violation to begin a new tranche while drift exists. The smoke test makes the violation observable to any agent or human inspecting the codebase cold.
...
- **Recenter the cockpit carefully.** Chat is the planned primary cockpit for planning, scoped work, review, approval, return, park, continuity inspection, and export requests. Tk remains the secondary operator surface for monitoring, traces, queues, deeper inspection, training/eval visibility, and emergency/manual control.
...
(This principle is the one carry-forward from the `_manifold-mcp` package's design — see [`_docs/migration/INCORPORATION_INVENTORY.md`](../migration/INCORPORATION_INVENTORY.md) §3.2.)

### 13.4 Chat workspace over the spine

The planned chat workspace is an **interface**, not a second brain.

Rules:

1. **Project truth still lives in the sidecar spine.** The DB, journal, traces, evidence, approvals, review packets, and generated metadata remain authoritative.
2. **Chat must act through existing routed surfaces.** A chat command becomes a projection read, envelope-routed intent, approval object, review packet request, export request, or tool invocation through the normal sidecar path.
3. **No transcript-only authority.** Approve/return/park from chat is valid only when it becomes a recorded review or approval object inside the sidecar.
4. **No second memory layer.** Free-floating transcript memory must not become a hidden project store competing with `data/sidecar.db`.
...
The binding contract at `contracts/BCC.md` is the sole long-form doctrine source. The **constraint registry** projects contract doctrine into addressable, queryable units so the contract gate can ask narrow questions and get focused answers.
...
- **Park Phase discipline** — RESOLVED (Decision 2026-05-11, see journal entry `journal_18ae7fbc35603af0_ec2ea642`): the Park Phase is now an explicit checklist in §12.2, contract-bound in `BCC.md §10.8`, historically interpretable through `BCC.md` Appendix A, and mechanically enforced by `smoke_test.py` drift-detection sections. No more conversational ritual.
...
- **Cold-team handoff doctrine** — RESOLVED: `_docs/continuity/WE_ARE_HERE_NOW.md`, `_docs/planning/NORTHSTARS.md`, `_docs/history/DEV_LOG.md`, and the `handoff` projection promote continuity into first-class, mechanically-enforced project state.
...
- **Manual closeout-id mirroring drift** — RESOLVED for latest parked tranche metadata: `close_tranche` now derives and writes `_docs/continuity/LATEST_PARKED_TRANCHE.json/.md` plus tranche-specific `_docs/history/tranches/Tn_CLOSEOUT_METADATA.json/.md` files so journal ids and CAS refs come from one generated source instead of hand-copied prose.
...
### Resolved at T10.1 (2026-05-15)

- **Chat-centered target ambiguity** — RESOLVED in doctrine: `TARGET_STATE.md` now separates the Prototype Target State from the Long-Term Evolutionary Target so future work does not sprawl under one fuzzy “chat workspace” idea.
- **Primary-cockpit confusion** — RESOLVED in doctrine: chat is now explicitly the planned primary cockpit and Tk the secondary operator surface, without weakening the existing Tk monitoring/training/proof surfaces.
- **Second-brain drift risk** — RESOLVED in doctrine: the chat workspace is now explicitly constrained to be a governed projection/action surface over the sidecar spine rather than a second memory or authority layer.
