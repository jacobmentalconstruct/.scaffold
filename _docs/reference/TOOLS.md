...
8 tools registered. The sidecar still keeps the runtime tool floor intentionally narrow. T8 added a training/evaluation substrate around those same tools, T9 proved installed vendability without expanding the belt prematurely, and the current registry includes one narrow audit surface for documentation drift: `doc_registry_audit`.
...
| `doc_registry_audit` | introspection | Audit canonical doc registry health, alias use, and root clutter drift. | `doc_registry_audit` | Observe | [src/tools/doc_registry_audit.py](src/tools/doc_registry_audit.py) |
