...
- resources/list  → projection://<name> plus chat-cockpit review/closeout URIs
- resources/read  → reads a projection, review packet, or closeout artifact
- sidecar/submit  → submits non-tool envelopes such as
                    acknowledge_contract, tranche review intents, and
...
from src.orchestrators.closeout_orchestrator import derive_closeout_metadata
...
class MCPHandler:
    ...
    def _handle_initialize(self, params: dict) -> dict:
        return {
            ...
            "instructions": (
                ...
                "then read projection://tranche_review_gate and review://latest "
                "for review/park workflows, then use tools/call or sidecar/submit "
                "for bounded actions through the same spine."
            ),
        }
    ...
    def _handle_resources_list(self, params: dict) -> dict:
        ...
        resources.extend(self._review_resource_descriptors())
        resources.extend(self._closeout_resource_descriptors())
        return {"resources": resources}

    def _handle_resources_read(self, params: dict) -> dict:
        uri = params.get("uri", "")
        if uri.startswith("projection://"):
        ...
        if uri.startswith("review://"):
            return self._read_review_resource(uri)
        if uri.startswith("closeout://"):
            return self._read_closeout_resource(uri)
    ...
    def _review_resource_descriptors(self) -> list[dict]:
        resources = [
            {
                "uri": "review://latest",
                "name": "latest_review_packet",
                "description": "Latest mechanical tranche review packet (JSON + Markdown).",
                "mimeType": "application/json",
            }
        ]
        tranche_manager = getattr(self._state, "tranche_manager", None)
        latest_review = tranche_manager.get_latest_review_any() if tranche_manager else None
        if latest_review:
            resources.append(
                {
                    "uri": f"review://{latest_review.review_id}",
                    "name": latest_review.review_id,
                    "description": "Specific mechanical tranche review packet (JSON + Markdown).",
                    "mimeType": "application/json",
                }
            )
        return resources

    def _closeout_resource_descriptors(self) -> list[dict]:
        resources = [
            {
                "uri": "closeout://latest",
                "name": "latest_closeout_metadata",
                "description": "Latest derived tranche closeout metadata (JSON + Markdown).",
                "mimeType": "application/json",
            }
        ]
        try:
            latest = derive_closeout_metadata(self._state)
        except Exception:
            latest = {}
        journal_entry_uid = str(latest.get("journal_entry_uid") or "").strip()
        if journal_entry_uid:
            resources.append(
                {
                    "uri": f"closeout://{journal_entry_uid}",
                    "name": journal_entry_uid,
                    "description": "Specific derived tranche closeout metadata (JSON + Markdown).",
                    "mimeType": "application/json",
                }
            )
        return resources

    def _read_review_resource(self, uri: str) -> dict:
        review_id = uri[len("review://"):].strip()
        tranche_manager = getattr(self._state, "tranche_manager", None)
        if tranche_manager is None:
            raise _MCPError(-32602, "review resources unavailable: tranche manager not configured")
        if review_id == "latest":
            review = tranche_manager.get_latest_review_any()
        else:
            review = tranche_manager.get_review_by_id(review_id)
        if review is None:
            raise _MCPError(-32602, f"review resource unavailable: {uri}")

        try:
            json_text = self._state.blob_store.get_text(review.review_packet_json_ref)
            json.loads(json_text)
        except Exception as e:
            raise _MCPError(-32603, f"could not read review JSON payload for {review.review_id}: {e}")
        try:
            markdown_text = self._state.blob_store.get_text(review.review_packet_markdown_ref)
        except Exception as e:
            raise _MCPError(-32603, f"could not read review Markdown payload for {review.review_id}: {e}")

        return {
            "contents": [
                {
                    "uri": f"review://{review.review_id}#json",
                    "mimeType": "application/json",
                    "text": json_text,
                },
                {
                    "uri": f"review://{review.review_id}#markdown",
    ...
    def _read_closeout_resource(self, uri: str) -> dict:
        target = uri[len("closeout://"):].strip()
        try:
            metadata = derive_closeout_metadata(self._state, "" if target == "latest" else target)
        except Exception as e:
            raise _MCPError(-32602, f"closeout resource unavailable: {uri} ({e})")

        markdown_relpath = metadata["latest_markdown_path"] if target == "latest" else metadata["per_tranche_markdown_path"]
        markdown_path = Path(self._state.sidecar_root) / markdown_relpath
        try:
            markdown_text = markdown_path.read_text(encoding="utf-8")
        except Exception as e:
            raise _MCPError(-32603, f"could not read closeout Markdown payload for {uri}: {e}")

        return {
            "contents": [
                {
                    "uri": f"closeout://{metadata['journal_entry_uid']}#json",
                    "mimeType": "application/json",
                    "text": safe_json_dumps(metadata, indent=2),
                },
                {
                    "uri": f"closeout://{metadata['journal_entry_uid']}#markdown",
                    "mimeType": "text/markdown",
                    "text": markdown_text,
                },
            ]
...
def _default_object_type(operation_intent: str) -> str:
    mapping = {
        ...
        "request_tranche_review": "tranche_review",
        "return_tranche_review": "tranche_review",
        "approve_tranche_review": "tranche_review",
