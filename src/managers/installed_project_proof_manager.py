...
from src.lib.doc_registry import root_alias_paths
...
class InstalledProjectProofManager:
    ...
    def create_fixture(self, *, reset: bool = False) -> dict[str, Any]:
        if reset and self._host_root.exists():
            self._remove_tree_with_retries(self._host_root)
    ...
    def _install_sidecar_copy(self) -> None:
        if self._installed_sidecar_root.exists():
            self._remove_tree_with_retries(self._installed_sidecar_root)
        self._installed_sidecar_root.mkdir(parents=True, exist_ok=True)
        source_root = Path(self._state.sidecar_root)
        include_files = [
            "README.md",
            *root_alias_paths(source_root),
            "smoke_test.py",
        ]
        include_dirs = ["src", "contracts", "config", "training_scenarios", "_docs"]
    ...
    def _run_install_state_checks(self) -> dict[str, Any]:
        ...
        smoke = self._run_python(
            ["smoke_test.py"],
            extra_env={"SIDE_CAR_INSTALLED_CHILD_SMOKE": "1"},
            timeout=420,
        )
    ...
    def _remove_tree_with_retries(self, path: Path, *, attempts: int = 8, delay_seconds: float = 0.5) -> None:
