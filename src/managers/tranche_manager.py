...
class TrancheManager:
    ...
    def handle_declare_tranche(
        ...
        if drift["issues"]:
            raise RuntimeError(
                "declare_tranche blocked until Park/continuity drift is resolved: "
                + "; ".join(str(item) for item in drift["issues"])
            )
    ...
    def get_latest_review_any(self) -> TrancheReviewPacket | None:
        row = self._store.query_one(
            """
            SELECT * FROM tranche_review_packets
            ORDER BY generated_at DESC
    ...
    def get_review_by_id(self, review_id: str) -> TrancheReviewPacket | None:
        row = self._store.query_one(
            """
            SELECT * FROM tranche_review_packets
            WHERE review_id = ?
    ...
    def _check(
        ...
        if item_id == "tranche_declared":
            ...
            return ("fail", "no current tranche — run tranche-declare")
        ...
        if item_id == "journal_entry_closed":
            if not tranche:
                return ("fail", "no current tranche")
