...
class Router:
    ...
    def dispatch(self, envelope: "SidecarEnvelope") -> "SidecarEnvelope":
        ...
        if not check.accepted:
            ...
            if self._state.blob_store is not None:
                rejected = rejected.with_payload_ref(
                    self._state.blob_store.put_json(
                        {
                            "error": check.reason,
        ...
        try:
            ...
            failed = envelope.with_status("failed")
            if self._state.blob_store is not None:
                failed = failed.with_payload_ref(
                    self._state.blob_store.put_json(
                        {"error": str(e), "operation_intent": envelope.operation_intent}
                    )
                )
            return failed
