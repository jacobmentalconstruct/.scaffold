...
class ContractAuthority:
    ...
    def check(self, envelope: "SidecarEnvelope") -> CheckResult:
        ...
        # 4. Authority check.
        self._ensure_authority_record(envelope.actor_id, granted_by="system:router_authority_seed")
    ...
    def _ensure_authority_record(
        self,
        actor_id: str,
        *,
        granted_by: str = "system:contract_ack_seed",
    ) -> None:
        row = self._store.query_one(
    ...
    def _check_hard_block(self, unit, envelope: "SidecarEnvelope") -> str | None:
        ...
        return _project_mutation_policy_error(
            self._state.project_root,
            self._state.sidecar_root,
            tool_name,
            arguments,
        )
...
def _tool_scope_matches(scope: dict, tool_name: str, arguments: dict) -> bool:
    ...
    if requested_path:
        actual_path = _normalize_scope_path(arguments.get("path", ""))
        if _normalize_scope_path(requested_path) != actual_path:
            return False

    requested_prefix = str(scope.get("path_prefix", "")).strip()
    if requested_prefix:
        actual_path = _normalize_scope_path(arguments.get("path", ""))
        if not actual_path.startswith(_normalize_scope_path(requested_prefix)):
            return False

    requested_entry_paths = scope.get("entry_paths")
    if requested_entry_paths is not None:
        expected = _normalize_entry_paths_from_scope(requested_entry_paths)
        actual = _normalized_scaffold_entry_paths(arguments)
...
def _project_mutation_policy_error(project_root: str | Path, sidecar_root: str | Path, tool_name: str, arguments: dict) -> str | None:
    ...
    requested_paths, error = _project_target_paths(tool_name, arguments)
    if error:
        return error

    project_root_path = Path(project_root).resolve()
    sidecar_root_path = Path(sidecar_root).resolve()
    sidecar_inside_project = False
    try:
        sidecar_root_path.relative_to(project_root_path)
        sidecar_inside_project = True
    except ValueError:
        sidecar_inside_project = False

    for requested_path in requested_paths:
        candidate = (project_root_path / requested_path).resolve()
        try:
            candidate.relative_to(project_root_path)
        except ValueError:
            return "host-project write escapes project_root"
        if sidecar_inside_project:
            try:
                candidate.relative_to(sidecar_root_path)
                return "project-targeted mutation may not write into the sidecar runtime subtree"
            except ValueError:
                continue
    return None


def _project_target_paths(tool_name: str, arguments: dict) -> tuple[list[str], str | None]:
    if tool_name == "text_file_writer":
        path = _normalize_scope_path(arguments.get("path", ""))
        if not path:
            return [], "text_file_writer requires a bounded project path"
        return [path], None
    if tool_name == "directory_scaffold":
        entry_paths = _normalized_scaffold_entry_paths(arguments)
        if entry_paths is None:
            return [], "directory_scaffold requires an entries manifest with bounded entry.path values"
        if not entry_paths:
            return [], "directory_scaffold requires at least one entry.path"
        return entry_paths, None
    return [], None


def _normalized_scaffold_entry_paths(arguments: dict) -> list[str] | None:
    ...
    for entry in entries:
        if not isinstance(entry, dict):
            return None
        path = _normalize_scope_path(entry.get("path", ""))
        if not path:
            return None
...
def _normalize_entry_paths_from_scope(raw: object) -> list[str] | None:
    if not isinstance(raw, list):
        return None
    normalized: list[str] = []
    for item in raw:
        path = _normalize_scope_path(item)
        if not path:
            return None
        normalized.append(path)
    return sorted(dict.fromkeys(normalized))
