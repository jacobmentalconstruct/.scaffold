...
WHAT IT DOES (T2.5): Reads the active tranche object + all accumulated decision
      ...
      structured data, writes _docs/history/tranches/T{n}_PARK_NOTES.md,
      creates and closes a tranche journal entry, and seals the active_tranche.
...
Design intent ("compile-and-seal, not reconstruct-and-write"):
  ...
  close_tranche reads that structured data and derives:
    - the Markdown park notes file (_docs/history/tranches/Tn_PARK_NOTES.md)
    - the blob hash (stored as evidence on the journal entry)
    - the tranche journal entry (kind='tranche', status='closed')
  The 5 Park Phase artifacts (BCC §10.8; historically contract §D) are produced atomically:
    (1) _docs/history/tranches/Tn_PARK_NOTES.md written
...
from src.lib.doc_registry import doc_path, doc_relpath, read_doc_text
...
class CloseoutOrchestrator:
    ...
    def handle_close_tranche(
        ...
        docs_dir = Path(state.sidecar_root) / "_docs" / "history" / "tranches"
...
def derive_closeout_metadata(state: "SidecarState", journal_entry_uid: str = "") -> dict:
    ...
    return {
        ...
        "park_notes_path": f"_docs/history/tranches/{notes_filename}",
        "park_notes_blob_ref": park_notes_blob_ref,
        "decisions_count": decisions_count,
        "files_changed_count": files_changed_count,
        "per_tranche_json_path": f"_docs/history/tranches/{per_json_name}",
        "per_tranche_markdown_path": f"_docs/history/tranches/{per_md_name}",
        "latest_json_path": doc_relpath(state.sidecar_root, "latest_parked_tranche_json"),
        "latest_markdown_path": doc_relpath(state.sidecar_root, "latest_parked_tranche_md"),
        "generated_at": now_iso(),
...
def evaluate_park_continuity_drift(state: "SidecarState") -> dict:
    """Return authoritative latest-park drift facts for declare/smoke checks."""
    latest = state.store.query_one(
        """
        SELECT entry_uid, title, status
        FROM journal_entries
        WHERE kind = 'tranche' AND status = 'closed'
        ORDER BY created_at DESC
        LIMIT 1;
    ...
    if latest is None:
        return {"ok": True, "issues": [], "latest_closed": {}}

    entry_uid = str(latest["entry_uid"] or "")
    title = str(latest["title"] or "")
    issues: list[str] = []
    if str(latest["status"] or "") != "closed":
        issues.append("latest closed tranche journal entry is not marked closed")

    latest_closeout_path = doc_path(Path(state.sidecar_root), "latest_parked_tranche_json")
    closeout_meta: dict = {}
    if latest_closeout_path.is_file():
        try:
            closeout_meta = json.loads(latest_closeout_path.read_text(encoding="utf-8"))
        except Exception as exc:
            issues.append(f"generated latest closeout metadata is unreadable: {exc}")
    else:
        issues.append("generated latest closeout metadata file is missing")

    if closeout_meta:
        if str(closeout_meta.get("journal_entry_uid") or "") != entry_uid:
            issues.append("generated latest closeout metadata does not resolve to the latest closed tranche entry")
        if title and str(closeout_meta.get("title") or "") != title:
            issues.append("generated latest closeout metadata title does not match the latest closed tranche title")

    where_now = read_doc_text(Path(state.sidecar_root), "where_we_are_now")
    expected_phrase = "Current tranche:" if state.tranche_manager.get_current() else "Next horizon:"
    if title and title not in where_now:
        issues.append("WE_ARE_HERE_NOW.md does not name the latest parked tranche")
    if expected_phrase not in where_now:
        issues.append(f"WE_ARE_HERE_NOW.md does not use the expected lifecycle wording ({expected_phrase})")
    if expected_phrase == "Next horizon:" and "Active horizon:" in where_now:
        issues.append("WE_ARE_HERE_NOW.md still uses ambiguous Active horizon wording")
