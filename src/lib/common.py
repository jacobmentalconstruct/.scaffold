...
def resolve_paths(sidecar_root: Path) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        contract_file=sidecar_root / "contracts" / "builder_constraint_contract.md",
...
def detect_sidecar_root(start: Path | None = None) -> Path:
    ...
    for candidate in [here] + list(here.parents):
        if candidate.name == ".scaffold" or (candidate / "contracts" / "builder_constraint_contract.md").is_file():
            return candidate
