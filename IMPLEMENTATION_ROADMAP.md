...
7. **Display via Tk UI** — a human launches `python -m src.app ui`, sees the State, Journal, Evidence, and Project Map panels populated from the same projections the agent reads.
...
- **DOCS (already exist or updated):** `README.md`, `ARCHITECTURE.md`, `IMPLEMENTATION_ROADMAP.md` (this file), `SOURCE_PROVENANCE.md`, `TOOLS.md`, `contracts/builder_constrant_contract.md`, `_docs/INCORPORATION_INVENTORY.md`
...
- `src/ui/`: main_window, state_panel, journal_panel, evidence_panel, project_map_panel (5 panels, no contracts_panel yet)
- Async/threading wiring in main_window for long-running envelope dispatches

**Non-goals:** no contracts_panel approval flow (T4); no live edit affordances beyond journal entry composition.

**Completion criteria:**
- `python -m src.app ui` opens the Tk window, shows all four panels populated.
- Status bar polls every 3s and shows last refresh time per projection.
- Human can compose a `create_journal_entry` envelope from the journal panel and submit it; entry appears after refresh.
- Human can click a file in the project map panel and see citing journal entries.
- Manual end-to-end: install → scan → open UI → see populated dashboard.

**Memory model status:** LTM has a human face. Both surfaces (UI and MCP) read identical projections.
