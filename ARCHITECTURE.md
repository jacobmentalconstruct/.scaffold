...
The binding contract at `contracts/builder_constrant_contract.md` is a long-form document. The **constraint registry** projects it into addressable, queryable units so the contract gate can ask narrow questions and get focused answers.
...
- **Approval UX:** Tkinter modal vs queue panel. Decide when building `contracts_panel` in T4.
